...
class OpticalFlowSolver:
    ...
    def save_individual_images(self, I_initial, I_final, I_computed, save_dir="optical_flow_results"):
        ...
        print(f"\nSaving individual images to {save_dir}/...")
        
        # Ensure images are in proper range [0,1] for saving
        I_initial_save = np.clip(I_initial, 0, 1)
        I_final_save = np.clip(I_final, 0, 1)
        I_computed_save = np.clip(I_computed, 0, 1)
        
        if len(I_initial.shape) == 3:
            # Save RGB images
            plt.figure(figsize=(8, 6))
            plt.imshow(I_initial_save)
            plt.axis('off')
            plt.title('Initial Image')
            plt.tight_layout()
            plt.savefig(f"{save_dir}/01_initial_image.png", dpi=300, bbox_inches='tight')
            plt.close()
            
            plt.figure(figsize=(8, 6))
            plt.imshow(I_final_save)
            plt.axis('off')
            plt.title('Final Image')
            plt.tight_layout()
            plt.savefig(f"{save_dir}/02_final_image.png", dpi=300, bbox_inches='tight')
            plt.close()
            
            # Save convection intermediate result
            ...
            I_conv_half_save = np.clip(I_conv_half, 0, 1)
            
            plt.figure(figsize=(8, 6))
            plt.imshow(I_conv_half_save)
            plt.axis('off')
            plt.title('Convection Result (T/2)')
            plt.tight_layout()
            plt.savefig(f"{save_dir}/03_convection_half.png", dpi=300, bbox_inches='tight')
            plt.close()
            
            plt.figure(figsize=(8, 6))
            plt.imshow(I_computed_save)
            plt.axis('off')
            plt.title('Computed Result')
            plt.tight_layout()
            plt.savefig(f"{save_dir}/04_computed_result.png", dpi=300, bbox_inches='tight')
            plt.close()
            
            # Save error image
            error = np.abs(I_final_save - I_computed_save)
            plt.figure(figsize=(8, 6))
            plt.imshow(error)
            plt.axis('off')
            ...
            plt.colorbar()
            plt.tight_layout()
            plt.savefig(f"{save_dir}/05_error_image.png", dpi=300, bbox_inches='tight')
            plt.close()
            ...
            for ch, name in zip(channels, channel_names):
                ...
                # Save each channel's progression
                fig, axes = plt.subplots(1, 5, figsize=(20, 4))
                
                # Initial
                axes[0].imshow(ch_data['initial'], cmap='gray', vmin=0, vmax=1)
                axes[0].set_title(f'{name} Initial')
                axes[0].axis('off')
                
                # Final
                axes[1].imshow(ch_data['final'], cmap='gray', vmin=0, vmax=1)
                axes[1].set_title(f'{name} Final')
                axes[1].axis('off')
                
                # Convection at T/2
                axes[2].imshow(ch_data['convection_half'], cmap='gray', vmin=0, vmax=1)
                axes[2].set_title(f'{name} Convection(T/2)')
                axes[2].axis('off')
                
                # Computed
                axes[3].imshow(ch_data['computed'], cmap='gray', vmin=0, vmax=1)
                axes[3].set_title(f'{name} Computed')
                axes[3].axis('off')
                
                # Error
                ch_error = np.abs(ch_data['final'] - ch_data['computed'])
                im = axes[4].imshow(ch_error, cmap='hot')
                axes[4].set_title(f'{name} Error')
                axes[4].axis('off')
                plt.colorbar(im, ax=axes[4])
                
                plt.tight_layout()
                plt.savefig(f"{save_dir}/06_{ch}_channel_progression.png", dpi=300, bbox_inches='tight')
                plt.close()
            ...
            plt.figure(figsize=(8, 6))
            plt.imshow(ch_data['initial'], cmap='gray', vmin=0, vmax=1)
            plt.axis('off')
            plt.title('Initial Image')
            plt.tight_layout()
            plt.savefig(f"{save_dir}/01_initial_image.png", dpi=300, bbox_inches='tight')
            ...
            plt.figure(figsize=(8, 6))
            plt.imshow(ch_data['final'], cmap='gray', vmin=0, vmax=1)
            plt.axis('off')
            plt.title('Final Image')
            plt.tight_layout()
            plt.savefig(f"{save_dir}/02_final_image.png", dpi=300, bbox_inches='tight')
            ...
            plt.figure(figsize=(8, 6))
            plt.imshow(ch_data['convection_half'], cmap='gray', vmin=0, vmax=1)
            plt.axis('off')
            plt.title('Convection Result (T/2)')
            plt.tight_layout()
            plt.savefig(f"{save_dir}/03_convection_half.png", dpi=300, bbox_inches='tight')
            ...
            plt.figure(figsize=(8, 6))
            plt.imshow(ch_data['computed'], cmap='gray', vmin=0, vmax=1)
            plt.axis('off')
            plt.title('Computed Result')
            plt.tight_layout()
            plt.savefig(f"{save_dir}/04_computed_result.png", dpi=300, bbox_inches='tight')
        ...
        print(f"Individual images saved successfully in {save_dir}/")

    def plot_results(self, I_initial, I_final, I_computed, save_plots=True, save_dir="optical_flow_results"):
        ...
        if save_plots:
            ...
            os.makedirs(save_dir, exist_ok=True)
            if len(I_initial.shape) == 3:
                plt.savefig(f"{save_dir}/00_complete_analysis_rgb.png", dpi=300, bbox_inches='tight')
                print(f"Complete RGB analysis plot saved to {save_dir}/00_complete_analysis_rgb.png")
            else:
                plt.savefig(f"{save_dir}/00_complete_analysis_grayscale.png", dpi=300, bbox_inches='tight')
                print(f"Complete grayscale analysis plot saved to {save_dir}/00_complete_analysis_grayscale.png")
        
        plt.show()
        
        # Save individual images
        if save_plots:
            self.save_individual_images(I_initial, I_final, I_computed, save_dir)
...
def main():
    ...
    try:
        ...
        solver.plot_results(I_initial, I_final, I_computed, save_plots=True, save_dir="optical_flow_results")
        
        # Save additional analysis files
        save_parameters_analysis(solver, save_dir="optical_flow_results")
        
        print("\n=== Algorithm completed successfully! ===")
        print("Results saved to 'optical_flow_results/' directory")
...
def save_parameters_analysis(solver, save_dir="optical_flow_results"):
    """Save detailed parameter analysis to files"""
    import os
    
    os.makedirs(save_dir, exist_ok=True)
    
    # Save parameters as text file
    with open(f"{save_dir}/parameters_summary.txt", "w") as f:
        f.write("=== Optical Flow Parameters Analysis ===\n\n")
        
        if hasattr(solver, 'channel_results'):
            if 'gray' in solver.channel_results:
                # Grayscale case
                params = solver.parameters['gray']
                ch_data = solver.channel_results['gray']
                
                f.write("Grayscale Channel Analysis:\n")
                f.write(f"Time split: Convection (0 → {solver.T/2}), Source ({solver.T/2} → {solver.T})\n")
                f.write(f"Convection equation: ∂I/∂t + u·∂I/∂x + v·∂I/∂y = 0\n")
                f.write(f"Source equation: ∂I/∂t = c + d·I\n\n")
                
                f.write("Flow Field Statistics:\n")
                f.write(f"  u (horizontal flow): mean={np.mean(params['u']):.6f}, std={np.std(params['u']):.6f}\n")
                f.write(f"  v (vertical flow): mean={np.mean(params['v']):.6f}, std={np.std(params['v']):.6f}\n")
                f.write(f"  max flow magnitude: {np.max(np.sqrt(params['u']**2 + params['v']**2)):.6f}\n\n")
                
                f.write("Source Parameters:\n")
                f.write(f"  c (additive term): mean={np.mean(params['c']):.6f}, std={np.std(params['c']):.6f}\n")
                f.write(f"  d (multiplicative term): mean={np.mean(params['d']):.6f}, std={np.std(params['d']):.6f}\n\n")
                
                error = np.abs(ch_data['final'] - ch_data['computed'])
                mse = np.mean((ch_data['final'] - ch_data['computed'])**2)
                f.write("Error Analysis:\n")
                f.write(f"  MSE: {mse:.8f}\n")
                f.write(f"  Max Error: {np.max(error):.8f}\n")
                f.write(f"  Mean Error: {np.mean(error):.8f}\n")
                
            else:
                # RGB case
                f.write("RGB Channel Analysis:\n")
                f.write(f"Time split: Convection (0 → {solver.T/2}), Source ({solver.T/2} → {solver.T})\n")
                f.write(f"Convection equation: ∂I/∂t + u·∂I/∂x + v·∂I/∂y = 0\n")
                f.write(f"Source equation: ∂I/∂t = c + d·I\n\n")
                
                for ch in ['r', 'g', 'b']:
                    ch_name = {'r': 'Red', 'g': 'Green', 'b': 'Blue'}[ch]
                    params = solver.parameters[ch]
                    ch_data = solver.channel_results[ch]
                    
                    f.write(f"{ch_name} Channel:\n")
                    f.write(f"  Flow Field Statistics:\n")
                    f.write(f"    u (horizontal): mean={np.mean(params['u']):.6f}, std={np.std(params['u']):.6f}\n")
                    f.write(f"    v (vertical): mean={np.mean(params['v']):.6f}, std={np.std(params['v']):.6f}\n")
                    f.write(f"    max flow magnitude: {np.max(np.sqrt(params['u']**2 + params['v']**2)):.6f}\n")
                    
                    f.write(f"  Source Parameters:\n")
                    f.write(f"    c (additive): mean={np.mean(params['c']):.6f}, std={np.std(params['c']):.6f}\n")
                    f.write(f"    d (multiplicative): mean={np.mean(params['d']):.6f}, std={np.std(params['d']):.6f}\n")
                    
                    ch_error = np.abs(ch_data['final'] - ch_data['computed'])
                    mse = np.mean((ch_data['final'] - ch_data['computed'])**2)
                    f.write(f"  Error Analysis:\n")
                    f.write(f"    MSE: {mse:.8f}\n")
                    f.write(f"    Max Error: {np.max(ch_error):.8f}\n")
                    f.write(f"    Mean Error: {np.mean(ch_error):.8f}\n\n")
    
    # Save flow field visualizations
    if hasattr(solver, 'parameters'):
        print(f"Saving flow field visualizations to {save_dir}/...")
        
        if 'gray' in solver.parameters:
            # Grayscale flow field
            params = solver.parameters['gray']
            
            fig, axes = plt.subplots(1, 3, figsize=(15, 5))
            
            # U component
            im1 = axes[0].imshow(params['u'], cmap='RdBu_r')
            axes[0].set_title('Horizontal Flow (u)')
            axes[0].axis('off')
            plt.colorbar(im1, ax=axes[0])
            
            # V component  
            im2 = axes[1].imshow(params['v'], cmap='RdBu_r')
            axes[1].set_title('Vertical Flow (v)')
            axes[1].axis('off')
            plt.colorbar(im2, ax=axes[1])
            
            # Flow magnitude
            flow_magnitude = np.sqrt(params['u']**2 + params['v']**2)
            im3 = axes[2].imshow(flow_magnitude, cmap='hot')
            axes[2].set_title('Flow Magnitude')
            axes[2].axis('off')
            plt.colorbar(im3, ax=axes[2])
            
            plt.tight_layout()
            plt.savefig(f"{save_dir}/07_flow_field_analysis.png", dpi=300, bbox_inches='tight')
            plt.close()
            ...
            for ch in ['r', 'g', 'b']:
                ...
                # U component
                im1 = axes[0].imshow(params['u'], cmap='RdBu_r')
                axes[0].set_title(f'{ch_name} Horizontal Flow (u)')
                axes[0].axis('off')
                plt.colorbar(im1, ax=axes[0])
                
                # V component
                im2 = axes[1].imshow(params['v'], cmap='RdBu_r')
                axes[1].set_title(f'{ch_name} Vertical Flow (v)')
                axes[1].axis('off')
                plt.colorbar(im2, ax=axes[1])
                ...
                flow_magnitude = np.sqrt(params['u']**2 + params['v']**2)
                im3 = axes[2].imshow(flow_magnitude, cmap='hot')
                axes[2].set_title(f'{ch_name} Flow Magnitude')
                axes[2].axis('off')
                plt.colorbar(im3, ax=axes[2])
                
                plt.tight_layout()
                plt.savefig(f"{save_dir}/07_{ch}_flow_field_analysis.png", dpi=300, bbox_inches='tight')
                plt.close()
    
    print(f"Parameters analysis saved to {save_dir}/parameters_summary.txt")
    print(f"Flow field visualizations saved to {save_dir}/")
